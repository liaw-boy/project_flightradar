...
// Shim: route/airport data now lives in in-memory staticMaps, not MongoDB
const AirportDictionary = {
    countDocuments: () => Promise.resolve(staticMaps.airportCount()),
    bulkWrite: (ops) => {
        for (const op of ops) {
            const data = op.updateOne?.update?.$set || op.updateOne?.update || {};
            const icao = (data.icao || data.ident || '').toUpperCase();
            if (icao) staticMaps.loadAirports([data]);
        }
        return Promise.resolve({ modifiedCount: ops.length });
    },
};
const RouteDictionary = {
    countDocuments: () => Promise.resolve(staticMaps.routeCount()),
    bulkWrite: (ops) => {
        for (const op of ops) {
            const data = op.updateOne?.update?.$set || op.updateOne?.update || {};
            if (data.callsign) staticMaps.loadRoutes([data]);
        }
        return Promise.resolve({ modifiedCount: ops.length });
    },
};
