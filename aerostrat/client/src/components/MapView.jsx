    ...
    useEffect(() => {
        ...
        function animate(time) {
            ...
            if (canvasLayerRef.current) {
                ...
                const zoom = map.getZoom();
                // Physical-pixel normalizer — keeps stroke widths device-independent (FR24 style)
                const dpr = window.devicePixelRatio || 1;
                ...
                for (let i = 0; i < Math.min(maxDraw, renderQueue.length); i++) {
                    ...
                    if (drawSize <= 6) {
                        // ── Tier 1: Tactical Dot — border in physical pixels ──
                        const dotR = Math.max(2, drawSize / 2);
                        const borderPx = 1.5 / dpr; // 1.5 physical pixels on every device
                        ctx.save();
                        ctx.globalAlpha = opacity;
                        ctx.fillStyle = 'rgba(0,0,0,0.7)';
                        ctx.beginPath();
                        ctx.arc(ptX, ptY, dotR + borderPx, 0, Math.PI * 2);
                        ctx.fill();
                        ...
                        if (dynImg && dynImg.complete) {
                            ...
                            if (vectorData) {
                                ...
                                ctx.lineJoin = 'round';
                                // Pass 1: dark shadow — 2 physical pixels on every device
                                ctx.strokeStyle = 'rgba(0,0,0,0.75)';
                                ctx.lineWidth = 2 / (dpr * canvasScale);
                                ctx.stroke(path);
                                // Pass 2: colored fill
                                ctx.fillStyle = altColor;
                                ctx.fill(path);
                                // Pass 3: crisp edge — 1 physical pixel (2 when selected)
                                ctx.strokeStyle = isSelected ? '#00ffff' : 'rgba(255,255,255,0.9)';
                                ctx.lineWidth = (isSelected ? 2 : 1) / (dpr * canvasScale);
                                ctx.stroke(path);
                                ctx.restore();
                            } else {
