...
- **🌍 即時航班追蹤**：全球航班的即時位置、高度、速度、航向、垂直速率、SPI (特殊位置識別) 等資訊。
- **🚀 極致的縮放效能 (300ms)**：防抖優化至 300ms，縮放地圖瞬間即刻取得最新資料，帶來無縫的拖曳體驗。
...
| `GET` | `/api/states` | 代理 OpenSky 即時飛機位置，根據 `lamin`, `lomin`, `lamax`, `lomax` 範圍查詢。 | **8 秒動態快取** |
...
A: OpenSky API 對於免費層或剛取得 token 的頻率有限制 (通常是幾秒一次)。如果過度頻繁縮放地圖會導致 429 Too Many Requests 限流。請**靜置畫面 2~3 分鐘**，待限流懲罰視窗過去後，飛機就會自動重新出現。
