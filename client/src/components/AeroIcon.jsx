...
export default function AeroIcon({ size = 24, bg = true, className = '', color, style }) {
    ...
    return (
        ...
        >
            <defs>
                {/* background gradient — dark navy with subtle top highlight */}
                <radialGradient id={`${id}-bgGrad`} cx="40%" cy="30%" r="70%">
                    <stop offset="0%" stopColor="#1e3a5f" />
                    <stop offset="100%" stopColor="#070e1a" />
                </radialGradient>
                {/* plane body gradient — sky blue with bright highlight */}
                <linearGradient id={`${id}-planeGrad`} x1="0%" y1="0%" x2="60%" y2="100%">
                    <stop offset="0%" stopColor="#7dd3fc" />
                    <stop offset="45%" stopColor="#38bdf8" />
                    <stop offset="100%" stopColor="#0369a1" />
                </linearGradient>
                {/* drop shadow filter for the plane */}
                <filter id={`${id}-shadow`} x="-20%" y="-20%" width="140%" height="140%">
                    <feDropShadow dx="0" dy="1" stdDeviation="1" floodColor="#000" floodOpacity="0.6" />
                </filter>
                {/* inner highlight shimmer on top edge of bg */}
                <linearGradient id={`${id}-shim`} x1="0%" y1="0%" x2="0%" y2="100%">
            ...
            {bg && (
                <>
                    {/* base background */}
                    <rect width="32" height="32" rx="6" fill={`url(#${id}-bgGrad)`} />
                    {/* outer stroke for depth */}
                    <rect width="32" height="32" rx="6" fill="none" stroke="#1e4976" strokeWidth="0.8" opacity="0.6" />
                    {/* top-edge shimmer */}
                    <rect width="32" height="32" rx="6" fill={`url(#${id}-shim)`} />
                </>
            )}

            {/* airplane silhouette with gradient + shadow */}
            <g
                transform="translate(16,16) rotate(90) scale(0.32) translate(-39.5, -35)"
                filter={`url(#${id}-shadow)`}
            >
                <path fill={`url(#${id}-planeGrad)`} d={A388_PATH} />
            </g>
        </svg>
    );
}
